...
[Collection(nameof(StorageServiceTestFixtureCollection))]
public class StorageServiceTest
{
    ...
    public async Task Upload()
    {
        ...
        storageClientMock.Setup(x => x.UploadObjectAsync(It.IsAny<string>(), 
            It.IsAny<string>(),
            It.IsAny<string>(), 
            It.IsAny<Stream>(), 
        ...
        var filePath = await service.UploadAsync(fileName, stream, contentType, CancellationToken.None);

        Assert.Equal(fileName, filePath);
        storageClientMock.Verify(x => x.UploadObjectAsync(storageOptions.BucketName, fileName, contentType, stream, It.IsAny<UploadObjectOptions>(), It.IsAny<CancellationToken>(), It.IsAny<IProgress<IUploadProgress>>()), Times.Once());
    }
    ...
    {
        var storageClientMock = new Mock<StorageClient>();
        storageClientMock.Setup(x => x.DeleteObjectAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<DeleteObjectOptions>(), It.IsAny<CancellationToken>()))
        ...
        var service = new StorageService(storageClientMock.Object, options);
        var fileName = _fixture.GetFileName();
        await service.DeleteAsync(fileName, CancellationToken.None);
        storageClientMock.Verify(x => x.DeleteObjectAsync(storageOptions.BucketName, fileName, It.IsAny<DeleteObjectOptions>(), It.IsAny<CancellationToken>()), Times.Once());
    }
}
